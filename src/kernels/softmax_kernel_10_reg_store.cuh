...
__global__ void softmax_kernel_10(scalar_t* __restrict__ a, scalar_t* __restrict__ b, int w, int h)
{
  int row = blockIdx.x;
  int ty = threadIdx.y;
  int warp_id = ty/32;
  ...
  float4 reg_array[cuda::ceil_div((WIDTH/4),BLOCK_DIM_X)];
  ...
  if (row < h)
  {
    float maxval = 0;
#pragma unroll URF
    for (int i = ty; i<WIDTH/4; i+=BLOCK_DIM_X)
    {
...
__global__ void softmax_kernel_reg_store(scalar_t* __restrict__ a, scalar_t* __restrict__ b, scalar_i totalRow, scalar_i totalCol) {
}