...
## Python 调用与 torch.softmax 对比

### 调用链

Python 调用自定义 CUDA kernel 的完整路径：

```
import softmax_cuda
    │
    ├─ dlopen("softmax_cuda.cpython-313-...so")   # 加载 cmake 构建的 .so
    ├─ dlsym(handle, "PyInit_softmax_cuda")       # 查找 pybind11 生成的入口符号
    └─ PyInit_softmax_cuda()                      # 执行模块初始化，填充 __dict__

cuda.softmax_cuda(x)
    │
    ├─ pybind11 cpp_function 类型转换              # Python tensor → torch::Tensor（零拷贝）
    ├─ softmax_cuda(torch::Tensor x)              # interface.cpp 入口，透传到 run_kernels.cu
    ├─ AT_DISPATCH_FLOATING_TYPES                 # 运行时 dtype 分发 → 编译期 scalar_t 实例化
    ├─ softmax_kernel<<<grid, block>>>()          # 异步启动 GPU kernel
    └─ pybind11 类型转换返回                       # torch::Tensor → Python tensor（零拷贝）
```

`torch::Tensor` 是 8 字节轻量句柄，Python 侧与 C++ 侧共享同一块 GPU 显存，类型转换全程无数据拷贝。

### 与 torch.softmax 的性能对比

测试条件：float32，100 次重复，CUDA Event 计时，`torch/cuda` = torch耗时 / cuda耗时（> 1 表示 cuda 更快）

| Size | cuda mean | cuda median | torch mean | torch median | torch/cuda |
|------|----------:|------------:|-----------:|-------------:|-----------:|
| 128×1024  | — | — | — | — | 待测 |
| 128×4096  | — | — | — | — | 待测 |
| 128×16384 | — | — | — | — | 待测 |
...
> 运行 `./test_softmax.sh` 后结果写入 `benchmark_results/python_test_result.txt`。

### torch.softmax 实现差异

| 对比项 | 本项目 CUDA kernel（K8） | torch.softmax |
|--------|------------------------|---------------|
| 实现层 | 手写 CUDA，直接控制 grid/block | cuDNN / ATen 内核，框架自动选择 |
| 遍历次数 | 2-pass（Online Softmax） | 实现相关，通常 2–3 pass |
| 向量化 | float4（16 bytes/指令） | 框架优化，视 dtype 而定 |
| 规约方式 | warp shuffle + shfl_xor_sync | 库内部实现，不对外暴露 |
| 适用范围 | 固定 float32，2D/3D 输入 | 任意 dtype，任意维度 |
